...
- **Status:** done — T0–T4 shipped 2026-06-04.
- **Last updated:** 2026-06-04
- **Last touched on:** T4 landed — `cornell_sphere.gltf` (20,492 tris) + convergence CSV + 512² / 1024 spp reference
...
### T4 — Stanford bunny in Cornell Box (publishable artifact) ✅ DONE

**The real bunny shipped.** Sourced from the sibling
`~/src/morsel/examples/stanford-bunny.obj` (2,503 vertices, 4,968
triangles, Y-up, no normals) and committed at
`data/obj/stanford-bunny.obj`. A new OBJ parser + smooth-normal
computation in `examples/gen_cornell.rs` reads it at example-build
time (`include_str!`), area-weighted-accumulates face normals into
per-vertex smooth normals, scales 5× and translates `(+0.0825, 0,
+0.0075)` to centre on the floor, and emits
`data/gltf/cornell_bunny.gltf` (164 KB) — room + bunny, ~4,980
triangles, warm-clay Lambertian.

The level-5 icosphere stays available as a pure-procedural backup
scene at `data/gltf/cornell_sphere.gltf` (20,492 triangles); it's no
longer "the publishable artifact" but it's useful as the larger-mesh
BVH stress test (T2's 10× target lands there at 348.5×).

- [x] **Shipped glTFs** generated by `cargo run --example gen_cornell`:
      - `data/gltf/cornell_bunny.gltf` (164 KB, 4,980 triangles) — the
        canonical publishable artifact.
      - `data/gltf/cornell_sphere.gltf` (645 KB, 20,492 triangles) —
        the procedural test scene, kept for the BVH stress test.
- [x] **Convergence sweep on the bunny** at 128×128, max_spp 1024,
      reference_spp 4096 → `data/output/cornell_bunny_convergence.csv`
      (66 rows, 23.8s wall-clock). The shape matches M3 / T1 / sphere:
      PCG MIS+NEE drops 0.225 → 0.006 over 1024× more samples (37×
      reduction; theoretical √1024 = 32). MIS+NEE beats pure BSDF
      3-5× at every checkpoint:
      - PCG @ 1024 spp: misnee = **0.0062** vs bsdf = 0.0348 (5.6×)
      - Halton @ 1024 spp: misnee = 0.0071 vs bsdf = 0.0219 (3.1×)
      - Sobol @ 1024 spp: misnee = 0.0878 vs bsdf = 0.3768 (4.3×)
        (Sobol still trailing because of the dimension-limit issue
        documented in T2.)
      And on the procedural sphere
      (`data/output/cornell_sphere_convergence.csv`, also 66 rows, kept
      for cross-scene comparison): PCG MIS+NEE @ 1024 spp = 0.0060.
      The two scenes give bit-similar convergence numbers — exactly
      what we want as confidence the BVH + integrator aren't
      scene-dependent in pathological ways.
- [x] **Reference renders** at 512×512 / 1024 spp:
      - `data/output/cornell_bunny_reference.png` (330 KB) and `.exr`
        (8 MB) — 9.3s wall-clock on Apple M4. Soft red / green color
        bleeding on the bunny's body and contact-shadowed feet.
      - `data/output/cornell_sphere_reference.png` (330 KB) and `.exr`
        (8 MB) — 13.7s wall-clock. Kept alongside the bunny so the
        sphere has a published render too.
- [x] **`converge` subcommand** grew a `--scene PATH` flag mirroring
      `render --scene`. The `ConvergeConfig::scene_path: Option<PathBuf>`
      is `None` (default Cornell) or `Some(custom)`. `load_glb` errors
      surface as `io::Error::other(...)`.
- [x] **BVH 10× target absolutely lands here** — gated test
      `bvh_is_at_least_10x_faster_at_20k_triangles` renders
      cornell_sphere.gltf with BVH vs brute force at 256² / 64 spp.
      Measured on Apple M4: BVH = **217 ms**, brute = **75,716 ms**,
      **speedup = 348.5×**. The T2 test asserted ≥2× on the
      512-triangle scene as a no-regression floor; this is where the
      headline number actually shows up.

**Where the artifacts live.** `data/output/` is checked in for T4 as
the published artifact (bunny + sphere PNGs/EXRs/CSVs ≈ 16 MB total),
plus `data/obj/stanford-bunny.obj` (~410 KB) as the source mesh. If
`data/output/` grows past ~20 MB across future plans we'll move to
git-LFS or build-time-only outputs and trim history; for now the
commit is small enough to accept.

## Done when

- [x] Path tracer renders glTF-loaded triangle meshes natively and in the
      browser, with a SAH binned BVH on CPU and stack traversal on GPU.
- [x] The Cornell Box render is bit-similar (RMSE < 1e-3 vs. M3
      reference) at equal spp. (T1 measured 0.0049 vs. cornell_tris.)
- [x] The Stanford-bunny-in-Cornell scene renders cleanly, with a CSV +
      PNG + EXR shipped as the first publishable artifact of the
      triangle-mesh era.

Plan 0003 closes out — every milestone has shipped.
