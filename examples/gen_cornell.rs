...
fn main() {
    ...
    // PT-pbr-maps uplift: switched the bunny asset to the
    // UV-bearing variant generated by `morsel parameterize` so the
    // brushed-brass + normal-mapped scenes get real cylindrical
    // UVs instead of the old planar-XZ stretch that streaked the
    // body vertically.
    let bunny_obj = include_str!("../data/obj/stanford-bunny-uv.obj");
    let ObjParse {
        positions: bunny_raw_positions,
        uvs: bunny_raw_uvs,
        indices: bunny_indices,
    } = parse_obj_full(bunny_obj);
...
/// OBJ parse result carrying per-vertex UVs alongside positions
/// and indices.
///
/// UVs are read from `vt` entries and attached per position via
/// face `a/b` references. The bunny asset under
/// `data/obj/stanford-bunny-uv.obj` is the canonical UV-bearing
/// mesh; pre-morsel `stanford-bunny.obj` had no `vt` entries
/// and every UV came back as `(0, 0)`.
struct ObjParse {
    positions: Vec<[f32; 3]>,
    /// Length matches `positions`. When the OBJ has no `vt`
    /// entries or no `a/b` face attachments, every entry is
    /// `(0, 0)`.
    uvs: Vec<[f32; 2]>,
    indices: Vec<u32>,
}

fn parse_obj_full(src: &str) -> ObjParse {
    let mut positions: Vec<[f32; 3]> = Vec::new();
    let mut texcoords: Vec<[f32; 2]> = Vec::new();
    // Per-position UV slot. We attach UVs lazily as we see face
    // `a/b` references — when a position gets first referenced with
    // `vt` index `b`, we store `texcoords[b - 1]` here. Subsequent
    // references with mismatched `vt` indices are tolerated (the
    // first wins) since proper handling needs splitting the vertex.
    let mut position_uv: Vec<Option<[f32; 2]>> = Vec::new();
    ...
    for line in src.lines() {
        ...
        if let Some(rest) = line.strip_prefix("v ") {
            ...
            if parts.len() >= 3 {
                ...
                position_uv.push(None);
            }
        } else if let Some(rest) = line.strip_prefix("vt ") {
            let parts: Vec<&str> = rest.split_whitespace().collect();
            if parts.len() >= 2 {
                texcoords.push([
                    parts[0].parse().expect("OBJ vt u"),
                    parts[1].parse().expect("OBJ vt v"),
                ]);
            }
        } else if let Some(rest) = line.strip_prefix("f ") {
            // Faces can be `a b c` or `a/b c/d e/f` or `a/b/c d/e/f g/h/i`.
            // Parse both the vertex index and the optional vt index.
            let parts: Vec<&str> = rest.split_whitespace().collect();
            let mut tri_idx: Vec<u32> = Vec::with_capacity(parts.len());
            for p in &parts {
                let mut split = p.split('/');
                let v: i32 = split.next().unwrap().parse().expect("OBJ face index");
                let idx = if v > 0 {
                    v as u32 - 1
                } else {
                ...
                tri_idx.push(idx);
                // Optional `vt` index — attach to the position slot.
                if let Some(vt_field) = split.next() {
                    if !vt_field.is_empty() {
                        let vt: i32 = vt_field.parse().expect("OBJ face vt index");
                        let vt_idx = if vt > 0 {
                            vt as usize - 1
                        } else {
                            (texcoords.len() as i32 + vt) as usize
                        };
                        if vt_idx < texcoords.len() && (idx as usize) < position_uv.len() {
                            position_uv[idx as usize].get_or_insert(texcoords[vt_idx]);
                        }
                    }
                }
            }
...
/// PT-mr-map: `data/gltf/cornell_metal_bunny.gltf` — brushed-brass
/// metallic-roughness texture mapped via the morsel-baked
/// cylindrical UVs on the bunny (`stanford-bunny-uv.obj`). U
/// wraps around the body once; we tile ×3 horizontally so the
/// brushed streaks read at a sensible density across the body
/// silhouette. Seam at the back of the bunny (atan2 wrap).
fn build_cornell_brushed_metal_bunny(
    ...
    // Cylindrical UVs × (3, 1) tiling so the brushed streaks read
    // at a sensible density without smearing.
