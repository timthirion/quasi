...
pub struct GpuMaterial {
    ...
    /// Henyey-Greenstein asymmetry parameter. `0.0` = isotropic.
    /// Emitted to `extras.phase_g` when non-zero.
    pub phase_g: f32,
...
mod tests {
    ...
    fn gpu_struct_layout_matches_wgsl() {
        ...
        // `cloud_radius` (→ 76); PT-hg added `phase_g` (→ 80). The
        // runtime `Material` in `mesh.rs` lives separately and stays
        // std430-padded at 96 bytes.
